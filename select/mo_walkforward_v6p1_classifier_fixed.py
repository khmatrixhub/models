...
def main():
    ap = argparse.ArgumentParser(description="Walk-forward v6.1 (fixed): matches v5 with single file; supports S2 with different counts.")
    ap.add_argument("--mo-file", required=True)
    ap.add_argument("--strategy1-file", required=True)
    ap.add_argument("--strategy2-file", default=None, help="Optional Strategy 2 file; if omitted, S2 = inverse of S1 (same counts).")
    ...
    ap.add_argument("--out-base", default="wf_v61_fixed")
    args = ap.parse_args()
    ...
    # >>> FIX #2: drop any rows with missing MO features (exactly v5 behavior after shift+dropna)
    feat_cols = [c for c in df.columns if c.startswith(f"{args.prefix}delta_")]
    df.dropna(subset=feat_cols, inplace=True)
    df.reset_index(drop=True, inplace=True)
    if df.empty:
        raise RuntimeError("Merged dataset is empty after alignment.")

    dates = df[args.date_col].values
    s1_mean = df["s1_mean"].values; s1_count = df["s1_count"].values
    s2_mean = df["s2_mean"].values; s2_count = df["s2_count"].values
    pnl1 = s1_mean * s1_count; pnl2 = s2_mean * s2_count

    n = len(df); look = args.lookback_days
    if n < look + 3:
        look = max(20, n//4)

    # --- Global calibration using true pnl1/pnl2 ---
    calib_days = args.calibration_days or min(n//3, look*2)
    calib_days = max(calib_days, look+5)
    calib_end = min(n-2, calib_days)
    pnl_pos_sum = 0.0; pnl_neg_sum = 0.0

    for t in range(look, calib_end):
        Xtr = df.loc[t-look:t-1, feat_cols]; ytr = df.loc[t-look:t-1, "s1_mean"]
    ...
    log(f"GLOBAL mapping={global_mapping} (pos_sum={pnl_pos_sum:.4f}, neg_sum={pnl_neg_sum:.4f})")
    ...
    for t in range(look, n-1):
        ...
        if w.size == 0:
            ...
            Xt_z = ((Xt - mu) / sd).astype(float).fillna(0.0).values
            score_t = float(np.dot(Xt_z, w))
            choose1 = 1 if ((score_t < 0) if global_mapping=="S1_if_neg" else (score_t > 0)) else 0
    ...
    # Align to evaluation window (decisions apply to t+1)
    idx_start = look+1
    dates_eval = df[args.date_col].values[idx_start:]
    pnl1_eval = pnl1[idx_start:]; pnl2_eval = pnl2[idx_start:]
    c1_eval = s1_count[idx_start:]; c2_eval = s2_count[idx_start:]
    choose1_arr = np.array(choose1_list[1:])

    min_len = min(len(dates_eval), len(choose1_arr), len(pnl1_eval), len(pnl2_eval))
    dates_eval = dates_eval[:min_len]
    pnl1_eval = pnl1_eval[:min_len]; pnl2_eval = pnl2_eval[:min_len]
    c1_eval = c1_eval[:min_len]; c2_eval = c2_eval[:min_len]
    ...
    pd.DataFrame(feat_usage_rows).fillna(0.0).drop(columns=["date"], errors="ignore").sum().rename("count").to_frame().to_csv(out_feats)
    equity_plot(dates_eval, curves, out_equity,
                f"Walk-forward top-{args.top_k}, lookback={look} (GLOBAL mapping={global_mapping})")
    log(f"Saved: {out_equity}\n{out_decisions}\n{out_feats}")
