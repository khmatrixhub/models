...
def main():
    ap = argparse.ArgumentParser(description="Walk-forward top-k feature selection with PnL-calibrated sign mapping (1-step ahead) using MO deltas.")
    ...
    ap.add_argument("--out-base", default="mo_walkfwd_v3")
    args = ap.parse_args()
    ...
    for t in range(look, n-1):  # decision at t used for t+1
        train_slice = slice(t-look, t)   # [t-look ... t-1]
        Xtr_full = df.loc[train_slice, feat_cols]
        ytr_full = df.loc[train_slice, args.mean_col]
        pnl1_full = (df.loc[train_slice, args.mean_col] * df.loc[train_slice, args.count_col]).values
        pnl2_full = -pnl1_full

        # Per-feature corr on TRAIN only (using next-day target already aligned in df)
        stats = []
        for c in feat_cols:
            r, p = pearsonr(Xtr_full[c], ytr_full)
        ...
        Xtrz_full = zapp(Xtr_full[top], mu, sd).fillna(0.0).values
        ...
        # --- PnL-based mapping calibration ---
        # We need to evaluate mapping using 1-step-ahead within the training slice.
        # Scores at indices [t-look ... t-2] make decisions for [t-look+1 ... t-1],
        # which are inside the training window PnL arrays.
        if look < 2:
            s1_when_positive = True
            mapping = "S1_if_pos"
        else:
            # compute scores for train indices 0..look-1, but usable decisions are 0..look-2 -> next day 1..look-1
            scores_tr = (Xtrz_full @ w)
            # decisions for day j use score at j-1
            dec_pos = (scores_tr[:-1] > 0).astype(int)  # S1 when score>0
            dec_neg = (scores_tr[:-1] < 0).astype(int)  # S1 when score<0
            # corresponding next-day pnl arrays (align to 1..look-1)
            pnl1_next = pnl1_full[1:]
            pnl2_next = pnl2_full[1:]
            pnl_adapt_pos = dec_pos * pnl1_next + (1 - dec_pos) * pnl2_next
            pnl_adapt_neg = dec_neg * pnl1_next + (1 - dec_neg) * pnl2_next
            s1_when_positive = True if pnl_adapt_pos.sum() >= pnl_adapt_neg.sum() else False
    ...
    out_equity = os.path.join(args.out_dir, f"{args.out_base}_equity.png")
    equity_plot(use_dates, np.cumsum(pnl1[look+1:]), np.cumsum(pnl2[look+1:]), eqA, out_equity,
                f"Walk-forward top-{args.top_k}, lookback={args.lookback_days}d (PnL-calibrated)")
